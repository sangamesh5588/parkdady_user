...
class ChatEngine:
    ...
    def _prepare_training_data(self):
        ...
        for category, samples in training_samples.items():
            for sample in samples:
                self.training_data.append(self._preprocess_text(sample))
    ...
    def classify_intent(self, message: str) -> QueryIntent:
        """Classify user intent using multiple approaches"""
        ...
        tfidf_result = self._classify_by_tfidf(preprocessed)
        if tfidf_result:
            return tfidf_result
        ...
        return QueryIntent(
            ...
            confidence=0.3,
            keywords=[],
            suggested_response="I'm not sure about that. Let me connect you with a support agent."
        )
    ...
    def _classify_by_tfidf(self, preprocessed_message: str) -> Optional[QueryIntent]:
        """Classify using TF-IDF similarity"""
        if not self.vectorizer or not self.training_data:
            return None
        ...
        try:
            ...
            similarities = cosine_similarity(message_vector, training_vectors)[0]

            max_similarity_idx = np.argmax(similarities)
            max_similarity = similarities[max_similarity_idx]

            if max_similarity > 0.3:  # Threshold for similarity
                predicted_category = self.labels[max_similarity_idx]
                return QueryIntent(
                    category=predicted_category,
                    confidence=float(max_similarity),
                    keywords=[],
                    suggested_response=self._get_default_response(predicted_category)
                )
    ...
    def _classify_by_keywords(self, message: str) -> Optional[QueryIntent]:
        ...
        if category_scores:
            ...
            return QueryIntent(
                ...
                confidence=confidence,
                keywords=[k for k in keyword_map[best_category] if k in message_lower],
    ...
    def _get_default_response(self, category: QueryCategory) -> str:
        ...
        defaults = {
            QueryCategory.BOOKING: "I can help you with booking-related questions. Please provide more details about your booking inquiry.",
            QueryCategory.PAYMENT: "I can assist with payment and billing questions. What specific payment issue are you facing?",
